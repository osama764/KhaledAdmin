      ...
      (snapshot) => {
        ...
        snapshot.forEach((childSnapshot) => {
          ...
          devicesArray.forEach((device, i) => {
            ...
            let upArrow = `<i class="fa-solid fa-4x arrow-up fa-caret-up" style="color:${
              device.status_upArrow == "1" ? "#32e072" : "red"
            }"></i>`;
            let downArrow = `<i class="fa-solid fa-4x arrow-down fa-caret-down" style="color: ${
              device.status_downArrow == "1" ? "#32e072" : "red"
            }"></i>`;
            ...
            console.log(arrows)
            let icons = arrows.querySelectorAll(
              `.arrow-up, .arrow-down`
            );
            ...
            icons.forEach((icon, index) => {
                ...
                icon.addEventListener("mousedown", () => {
                  const statusType = getStatusTypePush(index);
                  const newStatus = "1";
                  updateDeviceStatusPush(
                    childSnapshot.key,
                    i,
                    statusType,
                    newStatus
...
function getStatusTypePush(index) {
  switch (index) {
    case 0:
      return "status_upArrow";
    case 1:
      return "status_downArrow";
...
function updateDeviceStatusPush(roomKey, deviceIndex, statusType, newStatus) {
  // Update status in Firebase
  const updates = {};
  updates[`Rooms/${roomKey}/devicesPush/${deviceIndex}/${statusType}`] =
    newStatus;
  firebase.database().ref().update(updates);
}
